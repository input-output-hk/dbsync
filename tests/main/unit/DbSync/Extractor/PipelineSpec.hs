...
module DbSync.Extractor.PipelineSpec (spec) where
...
spec :: Spec
spec = describe "DbSync.Extractor.Pipeline" $ do
...
runPipelineTwoBlocks extractors block1 block2 = withTestIngestStores $ \utxoStore dedupStores -> do
  ...
  let env2 = mkTestPipelineEnv resolver (mkTestWriter wrRef2) extractors
