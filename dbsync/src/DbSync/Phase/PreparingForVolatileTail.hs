...
-- "complete" markers.
module DbSync.Phase.PreparingForVolatileTail
  ( run
  ) where
...
import qualified DbSync.Phase.PreparingForVolatileTail.Backfill as Backfill
import qualified DbSync.Phase.PreparingForVolatileTail.Indexes as Indexes
...
import qualified DbSync.Phase.PreparingForVolatileTail.Sequences as Sequences
...
runDdl conn ddl = do
  ...
  case result of
    Right () -> pure ()
    Left  e  -> panic $ "PreparingForVolatileTail: " <> show e <> " for " <> ddl
