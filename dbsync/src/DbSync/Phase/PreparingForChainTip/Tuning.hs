...
--
-- 'PreparingForChainTip.run' opens one dedicated control connection
...
-- transaction. Each step in 'PreparingForChainTip.run' goes through
...
module DbSync.Phase.PreparingForChainTip.Tuning
  ( PrepTuning (..)
  , defaultPrepTuning
  , setPrepSessionGUCs
...
setPrepSessionGUCs conn t = do
  ...
  case result of
    Right () -> pure ()
    Left  e  -> panic $ "PreparingForChainTip.Tuning: " <> show e
