...
--
-- Runs at the start of 'DbSync.Phase.PreparingForVolatileTail.run' while
...
module DbSync.Phase.PreparingForVolatileTail.PreResolveIndexes
...
createPreResolveIndexes tracer conn =
  for_ (zip [1 :: Int ..] preResolveIndexStatements) $ \(i, ddl) ->
    timedTrace_ tracer "PreparingForVolatileTail"
...
runDdl conn ddl = do
  ...
  case result of
    Right () -> pure ()
    Left  e  -> panic $
      "PreparingForVolatileTail.PreResolveIndexes: " <> show e <> " for " <> ddl
