...
module DbSync.Phase.FollowingChainTip.RollbackSpec
  ( spec
  , schemaWalkSpec
  , cascadeSpec
  ) where
...
import DbSync.Phase.Boot (mkCardanoPoint)
...
-- ---------------------------------------------------------------------------
-- Tables and extractors mirror the FollowingChainTipSpec setup so the
