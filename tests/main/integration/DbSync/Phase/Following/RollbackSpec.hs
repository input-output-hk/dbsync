    ...
    it "advances dbsync_sync_state.last_committed_slot to the target" $ do
      ...
      _ <- queryTestDb $
        "INSERT INTO " <> tdName syncStateTableDef
          <> " (schema_version_applied, ledger_enabled, schema_fingerprint, extractors, network_magic, network_name, utxo_consumed_by_tx_id, utxo_strategy)"
          <> " VALUES (1, false, 'test-fp', '{}', 42, 'magic-42', true, 'archive') ON CONFLICT (id) DO NOTHING;"
