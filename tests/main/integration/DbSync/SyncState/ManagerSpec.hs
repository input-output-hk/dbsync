...
-- | Integration tests for 'DbSync.SyncState.Manager.commitEpoch'.
--
...
module DbSync.SyncState.ManagerSpec (spec) where
...
import DbSync.SyncState.Manager (commitEpoch)
import DbSync.Db.Loader (LoaderStream (..), closeLoaderStream, mkLoaderStream)
import DbSync.App.Env (LoaderWithControl (..))
  ...
  before_    resetFixtures $ do
    ...
    it "flushes bulk data AND advances dbsync_sync_state atomically" $ do
      ...
      withControlConnection $ \ctrl -> do
        runAppM ctrl (seedSyncState 1 testFp False [] 42 "magic-42" True "archive")
