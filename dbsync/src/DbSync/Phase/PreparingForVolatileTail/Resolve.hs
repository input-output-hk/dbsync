...
module DbSync.Phase.PreparingForVolatileTail.Resolve
  ( resolveForeignKeys
  ) where
...
resolveForeignKeys tracer conn = do
  n1 <- timedTrace tracer "PreparingForVolatileTail" "resolve tx_in.tx_out_id" $
          runStmt conn resolveTxInStmt
  n2 <- timedTrace tracer "PreparingForVolatileTail" "resolve collateral_tx_in.tx_out_id" $
          runStmt conn resolveCollateralTxInStmt
  n3 <- timedTrace tracer "PreparingForVolatileTail" "resolve reference_tx_in.tx_out_id" $
          runStmt conn resolveReferenceTxInStmt
  n4 <- timedTrace tracer "PreparingForVolatileTail" "resolve tx_out.consumed_by_tx_id" $
...
runStmt conn stmt = do
  ...
  case result of
    Right n -> pure n
    Left  e -> panic $ "PreparingForVolatileTail.Resolve: " <> show e
