...
redeemerTableDef = TableDef
  ...
  , tdColumns =
      ...
      , ColumnDef "purpose"          PgText    False
...
-- is a PostgreSQL-managed @GENERATED BY DEFAULT AS IDENTITY@ and the
-- row layer never threads an 'IdCounter' for it.
...
-- | COPY row for @extra_key_witness@. The @id@ column is omitted
-- because the table declares it as @GENERATED BY DEFAULT AS IDENTITY@
-- and the matching @COPY@ column list excludes it.
encodeExtraKeyWitnessCopy :: ExtraKeyWitness -> ByteString
encodeExtraKeyWitnessCopy ekw =
  buildCopyRow
