...
-- 'PreparingForVolatileTail' and 'FollowingChainTip' land — having the
  ...
  before_    truncateSyncState $ do
    ...
    describe "seedSyncStateStmt" $ do
      it "inserts the singleton row" $
        withTestConnection $ \conn -> do
          runStatement conn (1, "test-fp", False, [], 42, "magic-42", True, "archive") seedSyncStateStmt
          row <- runStatement conn () readSyncStateStmt
      ...
      it "is idempotent (ON CONFLICT DO NOTHING)" $
        withTestConnection $ \conn -> do
          ...
          runStatement conn (1, "test-fp", True, [], 42, "magic-42", True, "archive") seedSyncStateStmt
