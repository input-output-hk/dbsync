...
--
-- Per the LEDGER-PLAN, the worker reads blocks from the ledger
-- queue, applies them, and signals epoch boundaries via
-- 'leEpochReady'. Tests for 'applyBlockAndSnapshot' itself need a
-- real LSM session (Phase 6 fixtures), so what we cover here is the
-- /coordination/ around it: queue draining, the optional
-- 'leEpochWait' \"please reach epoch N\" signal, and the
-- backpressure shape.
--
-- We exercise these via 'runLedgerWorkerWith', a hook-based variant
-- that lets tests inject a fake @applyBlockAndSnapshot@ rather than
-- pulling the LSM construction in. The production entry point
-- ('runLedgerWorker') is just @runLedgerWorkerWith realApply@.
...
import DbSync.Ledger.Worker (WorkerHooks (..), runLedgerWorkerWith)
...
spec :: Spec
spec = describe "runLedgerWorkerWith" $ do
  it "applies every block delivered to the ledger queue" $ do
    ...
    workerThread <- async $
      runLedgerWorkerWith (countingHooks callsRef Nothing)
  ...
  it "signals leEpochReady when the fake hook reports a new epoch" $ do
    ...
    workerThread <- async $
      runLedgerWorkerWith (countingHooks callsRef (Just onEpochAt2))
        queue epochReady epochWait
