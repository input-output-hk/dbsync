...
-- | Pure tests for 'DbSync.Ingest.Consumer.advanceReplay'.
--
-- The replay-progress state machine is the only piece of new logic
-- introduced by the resume-restart UX work that has interesting
-- behaviour worth covering. The rest of the consumer is exercised
-- by the existing 'IngestPipelineSpec' integration suite.
...
spec :: Spec
spec = describe "DbSync.Ingest.Consumer.advanceReplay" $ do
