...
generateCreateTable td =
  ...
  where
    ...
    -- expression).
    --
    -- Identity columns are emitted as
    -- @"name" <type> GENERATED BY DEFAULT AS IDENTITY@. The 'BY DEFAULT'
    -- variant lets operators override during restore-from-dump while
    -- the steady-state path lets PostgreSQL allocate from the backing
    -- sequence.
