...
module DbSync.Phase.PreparingForVolatileTail.Backfill
  ( backfillTxColumns
  , applyDepositPending
...
-- 'DbSync.Phase.PreparingForVolatileTail.Resolve.resolveForeignKeys' so
...
backfillTxColumns tracer conn = do
  n1 <- timedTrace tracer "PreparingForVolatileTail" "backfill phase-2 tx.fee" $
          runRowsAffected conn backfillPhaseTwoFeeStmt
  n2 <- timedTrace tracer "PreparingForVolatileTail" "backfill Byron tx.fee" $
          runRowsAffected conn backfillByronFeeStmt
  n3 <- timedTrace tracer "PreparingForVolatileTail" "backfill phase-2 tx.deposit" $
          runRowsAffected conn backfillPhaseTwoDepositStmt
  n4 <- timedTrace tracer "PreparingForVolatileTail" "backfill valid-contract tx.deposit" $
...
applyDepositPending tracer conn = do
  n1 <- timedTrace tracer "PreparingForVolatileTail" "apply pool_update.deposit" $
          runRowsAffected conn applyPoolUpdateDepositStmt
  n2 <- timedTrace tracer "PreparingForVolatileTail" "apply stake_registration.deposit" $
...
truncateDepositPending conn = do
  ...
  case result of
    Right () -> pure ()
    Left  e  -> panic $ "PreparingForVolatileTail.Backfill: " <> show e
