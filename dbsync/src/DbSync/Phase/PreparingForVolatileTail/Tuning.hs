...
--
-- 'PreparingForVolatileTail.run' opens one dedicated control connection
...
-- transaction. Each step in 'PreparingForVolatileTail.run' goes through
...
module DbSync.Phase.PreparingForVolatileTail.Tuning
  ( PrepTuning (..)
  , defaultPrepTuning
  , setPrepSessionGUCs
...
-- | Issue the @SET@ statements that bring the connection up to the
-- requested 'PrepTuning'. Panics on driver failure — these are
-- unconditionally valid GUCs and a failure here points at a
-- connection-level problem worth surfacing immediately.
setPrepSessionGUCs :: Conn.Connection -> PrepTuning -> IO ()
setPrepSessionGUCs conn t = do
  result <- Conn.use conn (prepSessionGUCsSession t)
  case result of
    Right () -> pure ()
    Left  e  -> panic $ "PreparingForVolatileTail.Tuning: " <> show e
