...
# mithril-bootstrap-preview.sh
#
# Bootstrap cardano-node's database directory from a Mithril snapshot so we
# don't have to sync the preview testnet from genesis at the network layer.
# The script downloads the verified ImmutableDB + VolatileDB; cardano-node
# then replays the LedgerDB from genesis using that local immutable chain
# (which is orders of magnitude faster than fetching blocks from peers).
#
# Preview is served by the 'pre-release-preview' Mithril aggregator (status
# "Pre-release"), not 'release-preview' — that aggregator does not exist.
# See https://mithril.network/doc/manual/getting-started/network-configurations
# for the canonical list. The pre-release network is fed by the latest
# Mithril pre-release build, which the official mithril-client binary
# tolerates fine; if signature verification ever fails after a Mithril
# release rotation, upgrade mithril-client (the --mithril-version flag
# below) before suspecting the snapshot.
#
...
#   2. Download a verified Cardano DB snapshot from the pre-release-preview
...
#
# Usage:
#   ./scripts/mithril-bootstrap-preview.sh --db-dir <node-dir>                       # REQUIRED
#   ./scripts/mithril-bootstrap-preview.sh --db-dir <node-dir> --dry-run             # list available snapshots
#   ./scripts/mithril-bootstrap-preview.sh --db-dir <node-dir> --digest <d>          # pin to a specific snapshot
#   ./scripts/mithril-bootstrap-preview.sh --db-dir <node-dir> -y                    # auto-install mithril-client
#   ./scripts/mithril-bootstrap-preview.sh --db-dir <node-dir> --no-install          # never auto-install
#   ./scripts/mithril-bootstrap-preview.sh --db-dir <node-dir> --mithril-version <v> # pin mithril-client version
#
# --db-dir is mandatory. It points at the cardano-node directory (the folder
# containing config.json, topology.json, etc; for preview that's typically
# 'testnet-preview' — the name is irrelevant). The script will:
...
# Preview Mithril network parameters (pre-release-preview).
...
# genesis.vkey  -> decoded prefix: [127,73,124,161,6,137,131,213,...]
GENESIS_VERIFICATION_KEY="${GENESIS_VERIFICATION_KEY:-5b3132372c37332c3132342c3136312c362c3133372c3133312c3231332c3230372c3131372c3139382c38352c3137362c3139392c3136322c3234312c36382c3132332c3131392c3134352c31332c3233322c3234332c34392c3232392c322c3234392c3230352c3230352c33392c3233352c34345d}"
...
if [ -z "$NODE_DIR" ]; then
  ...
  echo "       containing config.json/topology.json; e.g. /usr/code/iog/testnet-preview)." >&2
  echo "" >&2
  usage 1
fi
...
log() {
  printf '[mithril-bootstrap-preview] %s\n' "$*"
}

err() {
  printf '[mithril-bootstrap-preview] ERROR: %s\n' "$*" >&2
}
...
log "This will take a while (snapshot is multi-GB, though smaller than mainnet)."
log ""
